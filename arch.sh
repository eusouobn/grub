...
list_disks_detailed() {
    ...
    # Coletar todos os discos físicos (excluindo loops)
    while IFS= read -r disk; do
        ...
        local disk_rota=$(lsblk -d -n -o ROTA "$disk_name" 2>/dev/null || echo "1")
        local disk_type="HDD"
        [[ "$disk_rota" == "0" ]] && disk_type="SSD"
        
        # Informações de partições
        local partitions=$(lsblk -n -o NAME,SIZE,FSTYPE,MOUNTPOINT,LABEL "$disk_name" 2>/dev/null | grep -v "^$" || echo "")
        local part_count=$(echo "$partitions" | grep -c . 2>/dev/null || echo "0")
...
select_disk() {
    ...
    # Listar discos e capturar o total
    local total_disks=0
    list_disks_detailed
    total_disks=$(lsblk -d -n -o NAME | grep -v "loop" | wc -l)
...
detect_partitions() {
    ...
    if [[ ${#linux_parts[@]} -gt 1 ]]; then
        ...
        for part in "${linux_parts[@]}"; do
            ...
            echo "  $part_num) /dev/$part - $part_size - $part_fs - $part_label - $part_mount"
        done
        read -p "> " PART_CHOICE
...
reinstall_grub() {
    ...
    if [[ "$BOOT_MODE" == "UEFI" ]]; then
        ...
        # Executar comando diretamente no chroot
        ...
        chroot "$MOUNT_DIR" grub-install --target=i386-pc \
                                         --boot-directory=/boot \
                                         "$SELECTED_DISK"
        local result=$?
